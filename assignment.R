...
my_intro <- make_introduction("Evan", "20")

# Create a variable `casual_intro` by substituting "Hello, my name is",
# with "Hey, I'm" in your `my_intro` variable.
# Hint: Take a look at the sub() function
...
minutes_in_two_days <- double(1440)
...
has_more_zs <- function(string1, string2) {
  ...
  zfinder1 <- str_count(string1, "z")
  zfinder2 <- str_count(string2, "z")
  if (zfinder1 > zfinder2) {
    print("The first string has more Zs.")
  }
  else if (zfinder1 == 0 && zfinder2 == 0) {
    print("Neither string contains the letter z.")
  }
  else if (zfinder1 == zfinder2) {
    return("The strings have the same number of Zs.")
  }
  else {
    return("The second string has more Zs.")
  }
}
# Create a variable `more_zs` by passing two strings of your choice to your
# `has_more_zs` function
more_zs <- has_more_zs("Zany zookeepers zapped zero zebras", "I like cereal")
more_zs <- has_more_zs("zzzz", "ZZZZ")
more_zs <- has_more_zs("ZZZ", "zzzz")
more_zs <- has_more_zs("Perry is a backseat gamer", "'I agree,' said Peter")
...
remove_digits <- function(strings) {
  vector <- c(strings)
  no_nums <- gsub("[[:digit:]]+", "", vector)
  return(no_nums)
}

# Demonstrate that your approach is successful by passing a vector of courses
# to your function. For example, remove_digits(c("INFO 201", "CSE 142"))
remove_digits(c("INFO 201", "CSE 142", "PSYCH 209", "ESS 101"))

### Vectors ###################################################################

# Create a vector `movies` that contains the names of six movies you like
movies <- c(
  "Saving Private Ryan", "Parasite", "Raise the Red Lantern",
  "Boyhood", "Dunkirk", "Baby Driver"
)
...
multiples_of_4[multiples_of_4 %% 4 == 0]
...
summary_info <- list(
  length = numbers_len, mean = numbers_mean, median =
    numbers_median
)
...
summarize_vector <- function(x) {
  list2 <- list(length = length(x), mean = mean(x), median = median(x))
  return(list2)
}
...
better_at_math <- grades$math_grades > grades$spanish_grades
better_at_math
